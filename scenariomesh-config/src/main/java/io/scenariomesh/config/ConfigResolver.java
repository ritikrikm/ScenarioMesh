...
/**
 * Resolves every ScenarioMesh configuration source through one precedence path.
 * No caller should independently interpret ScenarioMesh environment variables,
 * YAML values, aliases or defaults.
 */
public final class ConfigResolver {
    private final ConfigFileLoader fileLoader = new ConfigFileLoader();

    public ScenarioMeshConfig resolve(
    ...
    public ConfigResolution resolveDetailed(
            Path projectDirectory,
            Path buildDirectory,
            Map<String, String> properties,
            Map<String, String> environment) {
    ...
    private Duration durationValue(Object raw, Duration defaultValue, ConfigKey key) {
        ...
        try {
            return Duration.parse(String.valueOf(raw).trim());
        } catch (DateTimeParseException exception) {
            throw invalid(key, "must be an ISO-8601 duration such as PT30S or PT2M");
        }
    }

    private String stringValue(Object raw, String defaultValue) {
        return raw == null ? defaultValue : String.valueOf(raw).trim();
    }
