...
final class ConfigFileLoader {
    ...
    LoadedConfig load(Path projectDirectory, Path explicitFile) {
        ...
        if (selected == null) return new LoadedConfig(Optional.empty(), Map.of());
        ...
        if (!(document instanceof Map<?, ?> root)) throw invalid(selected, "root document must be a YAML mapping");
    ...
    private void flatten(String prefix, Map<?, ?> source, Map<String, Object> target, Path file) {
        for (Map.Entry<?, ?> entry : source.entrySet()) {
            ...
            if (value instanceof Map<?, ?> nested) flatten(path, nested, target, file);
    ...
    private void validateVersion(Map<String, Object> flattened, Path file) {
        ...
        if (raw == null) throw invalid(file, "'scenariomesh.configVersion' is required when a config file is present");
        int version;
        if (raw instanceof Number number) version = number.intValue();
        else {
            try { version = Integer.parseInt(String.valueOf(raw)); }
            catch (NumberFormatException exception) { throw invalid(file, "configVersion must be an integer"); }
        }
        if (version != SUPPORTED_VERSION) {
            throw invalid(file, "unsupported configVersion " + version + "; this runtime supports " + SUPPORTED_VERSION);
        }
    }
    ...
    private void validateKnownKeys(Map<String, Object> flattened, Path file) {
        ...
        if (!unknown.isEmpty()) throw invalid(file, "unknown configuration key(s): " + String.join(", ", unknown));
    }
